...
dir.create(file.path(dir_out,"Meth_control"))
dir_outfiles <- paste(dir_out,"/Meth_control",sep='')
# Set directory
setwd(dir_outfiles)
...
for (i in 1:length(samples_names)) {
  # Job name
  job_name <- paste(samples_names[i],"_Bismark_alignment_meth_control",sep='');
  ...
  # Command
  command <- paste("bismark --genome /vols/GPArkaitz_bigdata/DATA_shared/Genomes_Rocky/Ref_genome/RREMseq_controls/Meth/ -1 ", path, project, "/02_TRIMMED/" ,input1,
...
write.table(as.data.frame(logfile), paste(path, project, "/log/4_Bismark_align_", logdate, ".log", sep = ""), row.names = FALSE, eol = "\r")
